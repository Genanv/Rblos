-- Originally by shayman: This was my first time ever using the drawing library, there's probably 200 issues I haven't found yet. Please tell me if there are any problems.
...
function Object:GetQuad() -- Gets a table of positions for use in pretty much every ESP function. This also returns if the player is onscreen and will not return anything in the case that they aren't.
    ...
    local HealthBarFrom = ESP:GetScreenPosition((Pivot * CFrame.new(-X - HealthbarOffset, -Y, 0)).Position)
    local HealthBarFull = ESP:GetScreenPosition((Pivot * CFrame.new(-X - HealthbarOffset, Y, 0)).Position)
